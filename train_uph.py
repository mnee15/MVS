...
def save_model(model_rt, model_ph, model_uph, saved_dir, file_name):
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % val_every == 0:
            ...
            if uph_loss < best_loss:
                ...
                best_loss = uph_loss
                save_model(model_rt, model_ph, model_uph, saved_dir, args.experiment_name)
                counter = 0
            else:
                counter += 1
    ...
    save_model(model_rt, model_ph, model_uph, saved_dir, args.experiment_name + '_final')


def validation(model_rt, model_ph, model_uph, data_loader, device, criterion, epoch, args):
    ...
    return avrg_loss, Rt_loss, ph_loss, uph_loss
