...
from models import *
from models.mvsnet_nobaye import NLLKLLoss, mvsnet_loss_KL
from utils import *
import gc
import sys
import datetime
...
def train():
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            l2_loss, kl_loss = scalar_outputs['l2_loss'], scalar_outputs['kl_loss']
            ...
            if do_summary:
                ...
                if args.wandb:
                    wandb.log({
                            # 'Tr NLL Loss': round(nll_loss, 6),
                            # 'Tr KL Loss': round(kl_loss, 6),
                            'Tr Loss': round(loss, 6),
            ...
            print(
                'Epoch {}/{}, Iter {}/{}, train l2 loss = {:.3f}, train kl loss = {:.3f}, time = {:.3f}'.format(epoch_idx, args.epochs, batch_idx,
                                                                                     len(TrainImgLoader), l2_loss, kl_loss,
                                                                                     time.time() - start_time))
            # print(
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            if do_summary:
                ...
                if args.wandb:
                    wandb.log({
                            # 'Val NLL Loss': round(nll_loss, 6),
                            # 'Val KL Loss': round(kl_loss, 6),
                            'Val Loss': round(loss, 6),
            ...
            print(
                'Epoch {}/{}, Iter {}/{}, test l2 loss = {:.3f}, train kl loss = {:.3f}, time = {:.3f}'.format(epoch_idx, args.epochs, batch_idx,
                                                                                     len(TestImgLoader), l2_loss, kl_loss,
                                                                                     time.time() - start_time))
...
def train_sample(sample, kl_weight, nll_weight, detailed_summary=False):
    ...
    # nll_loss, kl_loss = model_loss(depth_est_disp, sigma, depth_gt_disp, mask, prob_volume, depth_value) ## NLLKLLoss for disp
    ...
    l2_loss, kl_loss = model_loss(depth_est, depth_gt, mask, prob_volume, depth_value)
    ...
    scalar_outputs = {'loss': loss, 'l2_loss':l2_loss, 'kl_loss': kl_loss}
