...
def train(args):
    ...
    # -- optimizer
    params = list(model_rt.parameters()) + list(model_ph.parameters()) + list(model_nval.parameters())
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_loader)) as pbar:
            for step, (imgs, exts, scs, nvals, ini_Rt, _) in enumerate(train_loader):  
                _, _, c, h, w = scs.shape
                ...
                loss = loss_Rt * args.criterion_weights[0] + loss_ph * args.criterion_weights[1] + loss_nval * args.criterion_weights[2]
                ...
                logging = {
                        ...
                        'Tr nval': round(loss_nval.item(), 4), 
                }
        ...
        if (epoch + 1) % val_every == 0:
            avrg_loss, Rt_loss, ph_loss, nval_loss = validation(model_rt, model_ph, model_nval, val_loader, device, criterion_val, epoch, args)
            if avrg_loss < best_loss:
                print(f"Best performance at epoch: {epoch + 1}")
                print(f"Save model in {saved_dir}")
                best_loss = avrg_loss
                save_model(model_rt, model_ph, model_nval, saved_dir, args.experiment_name)
                counter = 0
            else:
                counter += 1
    ...
    save_model(model_rt, model_ph, model_nval, saved_dir, args.experiment_name + '_final')


def validation(model_rt, model_ph, model_nval, data_loader, device, criterion, epoch, args):
    ...
    with torch.no_grad():        
        for step, (imgs, exts, scs, nvals, ini_Rt, _) in enumerate(data_loader):  
            _, _, c, h, w = scs.shape
            abs_Rt = torch.zeros((args.batch_size, args.view_num, 6))
            abs_Rt = abs_Rt.to(device)
            ...
            ph_loss += loss_ph
            nval_loss += loss_nval
            total_loss += loss_Rt * args.criterion_weights[0] + loss_ph * args.criterion_weights[1] + loss_nval * args.criterion_weights[2]
            cnt += 1
    ...
    return avrg_loss, Rt_loss, ph_loss, nval_loss
