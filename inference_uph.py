...
def test(args):
    ...
    with torch.no_grad():        
        for step, (imgs, exts, scs, uphs, ini_Rt, file_ids) in enumerate(test_loader):  
            _, _, c, h, w = scs.shape
            ...
            ph_loss += loss_ph
            uph_loss += loss_uph
            total_loss += loss_Rt + loss_ph + loss_uph
            cnt += 1
            ...
            pred_phs = pred_phs.detach().cpu().numpy()
            pred_uphs = pred_uphs.squeeze().detach().cpu().numpy() 
            ...
            for b in range(args.batch_size):
                files = file_ids[b]
                uph_path = os.path.join(args.pred_path, 'uph', f'{files[0]}.csv')
                np.savetxt(uph_path, pred_uphs[b] , delimiter=',')
                for v in range(args.view_num):
                    ...
                    np.savetxt(ph_path, pred_phs[b, v, :, :] , delimiter=',')
                    
        avrg_loss = total_loss / cnt
        Rt_loss, ph_loss, uph_loss = Rt_loss / cnt, ph_loss / cnt, uph_loss / cnt
    
    print(f'Inference || Loss: {round(avrg_loss.item(), 4)} || rt Loss: {round(Rt_loss.item(), 4)} || ph Loss: {round(ph_loss.item(), 4)} || uph Loss: {round(uph_loss.item(), 4)}')   
