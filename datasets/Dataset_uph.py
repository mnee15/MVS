...
class SynthDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        uph_path = os.path.join(self.dataset_path, 'uph_{}/{}_train/{:0>3}.csv'.format(self.pitch, scan, ref_view))
        uph = np.loadtxt(uph_path, delimiter=',')
        uph = np.expand_dims(uph, axis=0)
        uph = torch.from_numpy(uph)
        
        file_ids = [f'{scan}_{v:03d}' for v in view_ids]
        # file_id = f'{scan}_{view_ids[0]}'
        # return imgs, ext_matrices, mask
        return imgs, ext_matrices, scs, uph, ini_Rt, file_ids
