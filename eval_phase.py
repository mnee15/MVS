...
parser.add_argument('--refine', action='store_true', help='refine output')
...
def save_depth():
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            #                                                        outputs["sigma"], outputs["prob_volume"], outputs["view_weight"]):
            ...
            #     confidence_filename = os.path.join(args.outdir, 'sigma', filename + '.npy')
            #     prob_volume_filename = os.path.join(args.outdir, 'prob_vol', filename + '.npy')
            #     view_weight_filename = os.path.join(args.outdir, 'view_weight', filename + '.npy')
            #     os.makedirs(depth_filename.rsplit('/', 1)[0], exist_ok=True)
            #     os.makedirs(confidence_filename.rsplit('/', 1)[0], exist_ok=True)
            #     os.makedirs(prob_volume_filename.rsplit('/', 1)[0], exist_ok=True)
            #     os.makedirs(view_weight_filename.rsplit('/', 1)[0], exist_ok=True)
            #     # save depth maps
            ...
            #     np.save(confidence_filename, sigma)
            #     np.save(prob_volume_filename, prob_volume)
            #     np.save(view_weight_filename, view_weight)
...
def filter_depth(scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_intrinsics, ref_extrinsics = read_camera_parameters(
            os.path.join(args.testpath, 'cam/train/{:0>8}_cam.txt'.format(ref_view)))
        # load the reference image
        # ref_img = read_img(os.path.join(scan_folder, 'images/{:0>8}.jpg'.format(ref_view))) ##### image version
        ref_img = read_img(os.path.join(scan_folder, 'images/{:0>8}.jpg'.format(ref_view))) ##### phase version
...
if __name__ == '__main__':
    ...
    # for scan in scans:
    #     scan_id = int(scan[4:])
    #     scan_folder = os.path.join(args.testpath, f'{scan}_train')
